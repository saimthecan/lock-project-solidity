...
describe("Lock Contract", function () {
    ...
    before(async function () {
        ...
        const Token = await ethers.getContractFactory("canToken");
        ...
        await token.connect(owner).transfer(user2.address, ethers.utils.parseUnits("50", 18));

        await token.connect(user1).approve(lock.address, ethers.constants.MaxUint256);
        await token.connect(user2).approve(lock.address, ethers.constants.MaxUint256);
    });
    ...
    describe("Contract Functions", function () {
        ...
        it("user1 locks 10 tokens", async function () {
            ...
            userLocks[0] += 10;
            lockerCount++;
            await lock.connect(user1).lockTokens(ethers.utils.parseEther("10"), 60);
        ...
        it("user1 withdraws token", async function () {
            ...
            // 60 saniye bekleyin (EVM zamanını artırarak)
